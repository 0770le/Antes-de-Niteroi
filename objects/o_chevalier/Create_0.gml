...
can_riposte = true;
riposte_delay = room_speed;

function aerial_riposte() {
	//player exatamente acima do box do chavalier E com riposte   
	if ((distance_to_object(o_player) < attack_range) //limitando altura
	and (o_player.y < bbox_top) and (can_riposte)
	and ((abs(x - o_player.x) < sprite_width/2))) {
		can_riposte = false;
		riposte_delay = room_speed * 3;
		state = chevalier_states.AERIAL_RIPOSTE;
		image_index =0;
		alarm[7]= riposte_delay;
	}
