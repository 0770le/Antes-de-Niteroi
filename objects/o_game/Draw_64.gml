...
if (room != rm_menu) {
	...
	var	move = EaseOutBounce(current_frame, 0, 1, max_frames);
		...
		if ( ( (keyboard_check_pressed(vk_space) ) or (gamepad_button_check_pressed(0,gp_face1) ) ) and (!instance_exists(o_fade))) {
			//sound
			//audio_play_sound(snd_game_start, 15, false);
			
			//moveto the next room on the list
