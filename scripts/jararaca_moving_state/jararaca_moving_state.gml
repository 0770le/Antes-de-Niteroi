...
//change state
	...
	var t0 = tilemap_get_at_pixel(global.map, side() +1*facing, bbox_bottom + (TILE_SIZE*2)+1);	//2º bloco abaixo
	var t1 = tilemap_get_at_pixel(global.map, side() +1*facing, bbox_bottom + (TILE_SIZE*2)*2+1);	//3º bloco 
	var t2 = tilemap_get_at_pixel(global.map, side() +1*facing, bbox_bottom + (TILE_SIZE*2)*3+1);	//4º bloco
